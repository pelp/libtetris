...
struct tetris_input_state_t {
    tetris_inputs_t edge;
    tetris_inputs_t hold;
    bool            update;
};

struct tetris_hold_time_t {
    time_us_t rotate_cw;
...
struct tetris_t {
    ...
    // TODO: Add score and gravity system
    ...
    time_us_t
        delayed_auto_shift; // DAS: The time it takes for the piece movement to start repeating after initial movement
...
int shift(tetris_t *game, coord_t dx);
...
void add_transaction(transaction_list_t *list, tetris_params_t params) {
    ...
    list->transactions[list->used_size++] = (tetris_transaction_t){.params = params};
}
                ...
                for (int k = i; k >= 0; k--) {
                    for (int j = 0; j < game->framebuffer.width; j++) {
                        game->framebuffer.blocks[k * game->framebuffer.width + j] =
                            k == 0 ? PIECE_EMPTY : game->framebuffer.blocks[(k - 1) * game->framebuffer.width + j];
                    }
                }
            }
        }
...
tetris_input_state_t get_keys(tetris_t *game, tetris_params_t params) {
    ...
    return (tetris_input_state_t){.hold = hold, .edge = edge, .update = update};
}
...
TETRIS_API int tick(tetris_t *game, tetris_params_t params) {
    ...
    if (rc != -1 || params.inputs.soft_drop) {
        ...
        add_transaction(&game->transaction_list,
                        (tetris_params_t){.inputs = params.inputs, .delta_time = game->last_change});
